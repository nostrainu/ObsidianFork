...
function Library:CreateWindow(WindowInfo)
    ...
    function Window:AddTab(...)
        ...
        do
            if Side == "Footer" then
                ...
                TabButton = New("TextButton", {
                    ...
                    Text = "",
                    Parent = Side == "SidebarBottom" and SidebarBottom or Tabs,
                })
        ...
        function Tab:AddCenterGroupbox(...)
            local First = select(1, ...)
            if typeof(First) == "table" then
                local Info = First
                Info.Side = 3
    ...
    if ShowFABBlock then
        local UpdateMenu -- forward declaration for MakeFABDraggable
                ...
                if Dragging and IsHoverInput(Input) then
                    ...
                    if Delta.Magnitude > 5 then
                        if not Dragged and UpdateMenu then
                            UpdateMenu(false) -- collapse menu only when actual drag starts
                        end
                        Dragged = true
                    end
                    if Dragged then
        ...
        -- Start position: snapped to configured edge, centered vertically
        ...
        local FABButton = New("TextButton", {
            BackgroundColor3 = "BackgroundColor",
            Size = UDim2.fromOffset(48, 48), -- Fixed size so it doesn't stretch!
            Position = UDim2.new(0.5, 0, 0.5, 0), -- Center it so it can zoom from center
            AnchorPoint = Vector2.new(0.5, 0.5),
            Text = "",
            ZIndex = 102,
            Parent = FABContainer,
        ...
        -- Lucide "gem" icon inside the FAB
        local GemIcon = Library:GetCustomIcon("gem")
        ...
        -- Snapping & Draggability
        local IsDragged = MakeFABDraggable(FABContainer, FABButton)
        
        -- Quick Menu Buttons State
        local MenuExpanded = false
        local MenuButtons = {}
        local isHoveringMenu = false
        
        -- Auto-Fade: FAB ghosts out after idle period
        local fadeSeq = 0
        local isFaded = false
        local FADE_IDLE_TIME = 5   -- seconds before fading
        local FADE_OPACITY = 0.6   -- target transparency when faded
        ...
        -- Connect Enter/Leave on FABButton for hover zoom and hover menu state
        ...
        -- Function to update menu layout and tween buttons
        UpdateMenu = function(Expand)
            if MenuExpanded == Expand then return end
            MenuExpanded = Expand
            ...
            for idx, item in ipairs(MenuButtons) do
                ...
                if Expand then
                    ...
                    -- Snapping-aware radial math
                    local angle
                    if IsLeftHalf then
                        -- Fan out to the right (centered at 0 degrees, fanning from -70 to 70 degrees)
                        angle = math.rad((idx - (TotalButtons + 1) / 2) * 35)
                    else
                        -- Fan out to the left (centered at 180 degrees, fanning from 110 to 250 degrees)
                        angle = math.rad(180 - (idx - (TotalButtons + 1) / 2) * 35)
                    end
        ...
        local function BuildMenuButtons()
            ...
            -- 1. Add Tabs (visible or Settings)
            for _, Tab in ipairs(Window.TabsList) do
                if Tab.TabVisible or Tab.Name == "Settings" then
                    table.insert(Actions, {
                        Name = Tab.Name,
                        Icon = Tab.IconName,
                        Func = function()
                            Tab:Show()
                            Library:Toggle(true) -- Open main UI
                            UpdateMenu(false) -- Collapse menu on selection
                        end
                    })
                end
            end
            
            -- 2. Add Lock Drag action
            table.insert(Actions, {
                Name = "Lock Drag",
                ...
                Func = function(self)
                    ...
                    -- Visual indicator: glow outline when locked
            ...
            -- Build UI instances for each action
            for _, action in ipairs(Actions) do
                ...
                -- Hover tracking and zoom scale for mini-buttons
        ...
        -- Delayed build of menu buttons so TabsList is fully populated
        task.defer(BuildMenuButtons)
        
        -- Click / Double Click Interactions for FAB using Click-Counter State Machine
        local clickCount = 0
        local clickSequenceNum = 0
        local fabClicked = false -- guard to prevent click-outside conflict on mobile
        ...
        -- Dismiss on click outside with hover protection
