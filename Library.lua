...
do
    ...
    function CustomImageManager.DownloadAsset(AssetName: string, ForceRedownload: boolean?)
        ...
        local success, errorMessage = pcall(function()
            ...
            if url:match("raw.githubusercontent") then
                url = url .. "?t=" .. tostring(os.time())
            end
            writefile(AssetData.Path, game:HttpGet(url))
        end)
...
local function ApplySearchToTab(Tab, Search)
    ...
    if Tab.Name:lower():find(Search, 1, true) then
        ResetTab(Tab)
        return true
    end
    ...
    for _, Groupbox in Tab.Groupboxes do
        ...
        if VisibleElements > 0 then
            if Groupbox.Collapsible then
                ...
                Groupbox.Divider.Visible = true
                if Groupbox.CollapseButton then
                    Groupbox.CollapseButton.Rotation = 0
                end
            end
            Groupbox:Resize()
            HasVisible = true
        ...
        for SubTab, Visible in VisibleElements do
            ...
            if Visible > 0 then
                ...
                if Tabbox.ActiveTab == SubTab then
                    if Tabbox.Collapsible then
                        SubTab.Container.Visible = true
                        Tabbox.ButtonsFrame.Visible = true
                        if Tabbox.CollapseButton then
                            Tabbox.CollapseButton.Rotation = 0
                        end
                    end
                    SubTab:Resize()
...
local function ResetTab(Tab)
    ...
    for _, Groupbox in Tab.Groupboxes do
        ...
        if Groupbox.Collapsible then
            ...
            Groupbox.Divider.Visible = not Groupbox.Collapsed
            if Groupbox.CollapseButton then
                Groupbox.CollapseButton.Rotation = Groupbox.Collapsed and -90 or 0
            end
        end
    ...
    for _, Tabbox in Tab.Tabboxes do
        ...
        if Tabbox.Collapsible then
            Tabbox.ButtonsFrame.Visible = not Tabbox.Collapsed
            if Tabbox.ActiveTab then
                Tabbox.ActiveTab.Container.Visible = not Tabbox.Collapsed
            end
            if Tabbox.CollapseButton then
                Tabbox.CollapseButton.Rotation = Tabbox.Collapsed and -90 or 0
            end
        end
...
function Library:UpdateSearch(SearchText)
    if Library.ActiveTab and Library.ActiveTab.IsSearchResultsTab then
        return
    end
    Library.SearchText = SearchText

    local Search = SearchText:lower()
    local IsEmpty = Trim(Search) == ""
    if IsEmpty then
...
function Library:SetUpMarquee(Label: TextLabel, HoverFrame: GuiObject, Container: GuiObject)
    ...
    local function StopScroll()
        ...
        if Label:GetAttribute("OriginalAlignment") then
        ...
        elseif Container:GetAttribute("CenterCollapsible") then
            local LeftOffset = Container.Position.X.Offset
        ...
        if UIPadding then
            ...
            if Container:GetAttribute("Center") then
                local LeftOffset = Container:GetAttribute("LeftOffset") or 0
                local Collapsible = Container:GetAttribute("Collapsible")
                local margin = math.max(LeftOffset, Collapsible and 28 or 0)
                local extra = (LeftOffset == 30) and 4 or (LeftOffset == 24 and 6 or 12)
                SidePadding = margin > 0 and (margin + extra) or 12
            end
    ...
    local function StartScroll()
        if not Library.ScrollLongText then 
            Label.TextTruncate = Enum.TextTruncate.AtEnd
            Label.Size = UDim2.fromScale(1, 1)
        ...
        local UIPadding = Container:FindFirstChildOfClass("UIPadding")
        if UIPadding then
            local left = UIPadding.PaddingLeft.Offset + (UIPadding.PaddingLeft.Scale * StaticContainerWidth)
            local right = UIPadding.PaddingRight.Offset + (UIPadding.PaddingRight.Scale * StaticContainerWidth)
            StaticUsableWidth = StaticUsableWidth - (left + right)
        end
        ...
        if TextWidth > StaticUsableWidth then
            ...
            if IsCentered then
                local rightMargin = Collapsible and 28 or 0
                Container.Position = UDim2.fromOffset(LeftOffset, 0)
                Container.Size = UDim2.new(1, -LeftOffset - rightMargin, 0, 34)
            elseif Container:GetAttribute("CenterCollapsible") then
                Container.Size = UDim2.new(1, -LeftOffset - 28, 0, 34)
            end
    ...
    HoverFrame.MouseLeave:Connect(StopScroll)

    HoverFrame.InputBegan:Connect(function(Input)
        if Input.UserInputType == Enum.UserInputType.Touch or Input.UserInputType == Enum.UserInputType.MouseButton1 then
            if Library.IsMobile or UserInputService.TouchEnabled then
                IsHovered = true
                StartScroll()
...
do
    ...
    function Funcs:AddToggle(Idx, Info)
        ...
        local RealTextLabel = New("TextLabel", {
            BackgroundTransparency = 1,
            Size = UDim2.new(1, 0, 1, 0),
            Text = Toggle.Text,
            TextSize = 14,
        ...
        local LabelList = New("UIListLayout", {
            FillDirection = Enum.FillDirection.Horizontal,
            HorizontalAlignment = Enum.HorizontalAlignment.Right,
            Padding = UDim.new(0, 6),
            Parent = Label,
        ...
        local function UpdateTextContainerSize()
            local AddonsWidth = LabelList.AbsoluteContentSize.X
            LabelContainer.Size = UDim2.new(1, -40 - (AddonsWidth > 0 and (AddonsWidth + 6) or 0), 1, 0)
        end
        LabelList:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(UpdateTextContainerSize)
        task.defer(UpdateTextContainerSize)

        Library:SetUpMarquee(RealTextLabel, Button, LabelContainer)
        ...
        function Toggle:Display()
            ...
            if Toggle.Disabled then
                RealTextLabel.TextTransparency = 0.8
            ...
            TweenService:Create(RealTextLabel, Library.TweenInfo, {
                TextTransparency = Toggle.Value and 0 or 0.4,
            }):Play()
        ...
        if Toggle.Risky then
            RealTextLabel.TextColor3 = Library.Scheme.RedColor
            Library.Registry[RealTextLabel].TextColor3 = "RedColor"
        end
    ...
    function Funcs:AddViewport(Idx, Info)
        ...
        Library:GiveSignal(UserInputService.InputChanged:Connect(function(input)
            ...
            if not Viewport.Interactive or not Dragging or Pinching or not Viewport.Object then
                return
            end
...
function Library:CreateWindow(WindowInfo)
    ...
    do
        ...
        local HeaderLine = Library:MakeLine(MainFrame, {
        ...
        if SettingsIcon then
            ...
            Library:AddToRegistry(SettingsButton, {
                ImageColor3 = "FontColor",
            })

            SettingsButton.MouseEnter:Connect(function()
                TweenService:Create(SettingsButton, Library.TweenInfo, { ImageTransparency = 0.25 }):Play()
            end)
            SettingsButton.MouseLeave:Connect(function()
                TweenService:Create(SettingsButton, Library.TweenInfo, { ImageTransparency = 0.5 }):Play()
            end)
            SettingsButton.MouseButton1Click:Connect(function()
                local SettingsTab = Library.Tabs["Settings"]
                if SettingsTab then
                    SettingsTab:Show()
                end
            end)
    ...
    function Window:AddTab(...)
        ...
        if select("#", ...) == 1 and typeof(...) == "table" then
            ...
            Side = Info.Side or "Sidebar"
            if Info.Visible ~= nil then
                TabVisible = Info.Visible
            end
            if Info.ShowOnFloating ~= nil then
                ShowOnFloating = Info.ShowOnFloating
            end
        else
            Name = select(1, ...)
            Icon = select(2, ...)
        ...
        function Tab:AddGroupbox(Info)
            ...
            do
                ...
                local LeftOffset = BoxIcon and 24 or 0
                local RightMargin = (not Info.Center and Info.Collapsible) and 28 or 0
                local xOffset = -LeftOffset - RightMargin
                LabelContainer = New("Frame", {
                    BackgroundTransparency = 1,
                    ClipsDescendants = true,
                    Position = Info.Center and UDim2.fromOffset(0, 0) or UDim2.fromOffset(LeftOffset, 0),
                    Size = Info.Center and UDim2.new(1, 0, 0, 34) or UDim2.new(1, xOffset, 0, 34),
                    Parent = GroupboxHolder,
                })
                LabelContainer:SetAttribute("Center", not not Info.Center)
                LabelContainer:SetAttribute("Collapsible", not not Info.Collapsible)
                LabelContainer:SetAttribute("LeftOffset", LeftOffset)
                if Info.Center and Info.Collapsible then
                    LabelContainer:SetAttribute("CenterCollapsible", true)
                end
                ...
                local SidePadding = 12
                if Info.Center then
                    local margin = math.max(BoxIcon and 24 or 0, Info.Collapsible and 28 or 0)
                    SidePadding = margin > 0 and (margin + 6) or 12
                end
                New("UIPadding", {
            ...
            local Groupbox = {
                ...
                Divider = GroupboxDivider,

                Tab = Tab,
                DependencyBoxes = {},
                Elements = {},

                Collapsed = not not (Info.Collapsible and Info.DefaultCollapsed),
                Collapsible = not not Info.Collapsible,
            }
            ...
            if Info.Collapsible then
                ...
                Groupbox.CollapseButton = CollapseButton
            ...
            if GroupboxLabel and LabelContainer then
                Library:SetUpMarquee(GroupboxLabel, HeaderButton, LabelContainer)
            end
            ...
            function Groupbox:SetCollapsed(Collapsed, Instant)
                ...
                local TargetHeight = Collapsed and 34 or ((GroupboxList.AbsoluteContentSize.Y / Library.DPIScale) + 49)
                ...
                if Collapsed then
                    if Instant then
                        ...
                        if CollapseButton then CollapseButton.Rotation = TargetRotation end
                        GroupboxContainer.Visible = false
                        GroupboxDivider.Visible = false
                    else
                        ResizeTween = TweenService:Create(GroupboxHolder, Library.TweenInfo, { Size = UDim2.new(1, 0, 0, TargetHeight) })
                        if CollapseButton then
                            RotateTween = TweenService:Create(CollapseButton, Library.TweenInfo, { Rotation = TargetRotation })
                            RotateTween:Play()
                        end
                        ResizeTween:Play()
            ...
            function Groupbox:Resize()
                if Groupbox.Collapsed and not Library.Searching then
            ...
            Groupbox:SetCollapsed(Groupbox.Collapsed, true)
        ...
        function Tab:AddLeftGroupbox(...)
            ...
            if typeof(First) == "table" then
                ...
                local Name, IconName, Center, Collapsible, DefaultCollapsed = ...
                return Tab:AddGroupbox({ Side = 1, Name = Name, IconName = IconName, Center = Center, Collapsible = Collapsible, DefaultCollapsed = DefaultCollapsed })
            end
        end
        ...
        function Tab:AddRightGroupbox(...)
            ...
            if typeof(First) == "table" then
                ...
                local Name, IconName, Center, Collapsible, DefaultCollapsed = ...
                return Tab:AddGroupbox({ Side = 2, Name = Name, IconName = IconName, Center = Center, Collapsible = Collapsible, DefaultCollapsed = DefaultCollapsed })
            end
        end

        function Tab:AddMiddleGroupbox(...)
            local First = select(1, ...)
            if typeof(First) == "table" then
                local Info = First
                Info.Side = 3
                ...
                local Name, IconName, Center, Collapsible, DefaultCollapsed = ...
                return Tab:AddGroupbox({ Side = 3, Name = Name, IconName = IconName, Center = Center, Collapsible = Collapsible, DefaultCollapsed = DefaultCollapsed })
            end
        end
        ...
        function Tab:AddTabbox(Info)
            ...
            local HasHeader = not not Info.Collapsible
            local HeaderOffset = HasHeader and 35 or 0
            ...
            do
                ...
                if HasHeader then
                    ...
                    local BoxIcon = Info.IconName and Library:GetCustomIcon(Info.IconName)
                    if BoxIcon then
                        New("ImageLabel", {
                            Image = BoxIcon.Url,
                            ImageColor3 = BoxIcon.Custom and "WhiteColor" or "AccentColor",
                    ...
                    local LeftOffset = BoxIcon and 30 or (Info.Center and 0 or 12)
                    local RightMargin = (not Info.Center and Info.Collapsible) and 28 or 0
                    local xOffset = -LeftOffset - RightMargin
                    LabelContainer = New("Frame", {
                        BackgroundTransparency = 1,
                    ...
                    local SidePadding = 12
                    if Info.Center then
                        local margin = math.max(BoxIcon and 30 or 0, Info.Collapsible and 28 or 0)
                        SidePadding = margin > 0 and (margin + 4) or 12
                    end
                    New("UIPadding", {
                        PaddingLeft = UDim.new(0, SidePadding),
                        PaddingRight = UDim.new(0, SidePadding),
            ...
            if Info.Collapsible then
                ...
                Tabbox.CollapseButton = CollapseButton
            ...
            if TitleLabel and LabelContainer then
                Library:SetUpMarquee(TitleLabel, HeaderButton, LabelContainer)
            end

            local ResizeTween
            local RotateTween
            function Tabbox:SetCollapsed(Collapsed, Instant)
                if not Tabbox.Collapsible then return end
                Tabbox.Collapsed = Collapsed

                local TargetRotation = Collapsed and -90 or 0
                local ActiveTab = Tabbox.ActiveTab
                local TargetHeight = 34
                if not Collapsed and ActiveTab then
                    local SubTabList = ActiveTab.Container:FindFirstChildOfClass("UIListLayout")
                    local ListY = SubTabList and SubTabList.AbsoluteContentSize.Y or 0
                    TargetHeight = (ListY / Library.DPIScale) + HeaderOffset + 49
                end
                ...
                if Collapsed then
                    if Instant then
                        TabboxHolder.Size = UDim2.new(1, 0, 0, TargetHeight)
                        if Tabbox.CollapseButton then Tabbox.CollapseButton.Rotation = TargetRotation end
                        Tabbox.ButtonsFrame.Visible = false
                        if ActiveTab then ActiveTab.Container.Visible = false end
                    else
                        ResizeTween = TweenService:Create(TabboxHolder, Library.TweenInfo, { Size = UDim2.new(1, 0, 0, TargetHeight) })
                        if Tabbox.CollapseButton then
                            RotateTween = TweenService:Create(Tabbox.CollapseButton, Library.TweenInfo, { Rotation = TargetRotation })
                            RotateTween:Play()
                        end
                        ResizeTween:Play()
                        task.delay(Library.TweenInfo.Time, function()
                    ...
                    Tabbox.ButtonsFrame.Visible = true
                    if ActiveTab then ActiveTab.Container.Visible = true end
                    if Instant then
                        TabboxHolder.Size = UDim2.new(1, 0, 0, TargetHeight)
                        if Tabbox.CollapseButton then Tabbox.CollapseButton.Rotation = TargetRotation end
                    else
            ...
            function Tabbox:AddTab(Name, IconName)
                ...
                Library:SetUpMarquee(ButtonLabel, Button, LabelContainer)
                ...
                local Container = New("Frame", {
                    BackgroundTransparency = 1,
                    Position = UDim2.fromOffset(0, HeaderOffset + 35),
                    Size = UDim2.new(1, 0, 1, -(HeaderOffset + 35)),
                    Visible = false,
                    Parent = TabboxHolder,
                })
                ...
                function Tab:Show(IsInitial)
                    ...
                    if Tabbox.Collapsed and IsInitial ~= true then
                        Tabbox:SetCollapsed(false)
                    else
                        Tab:Resize()
                    end
                end
                ...
                function Tab:Resize()
                    ...
                    if Tabbox.Collapsed and not Library.Searching then
                        TabboxHolder.Size = UDim2.new(1, 0, 0, 34)
                    else
                        TabboxHolder.Size = UDim2.new(1, 0, 0, (List.AbsoluteContentSize.Y / Library.DPIScale) + HeaderOffset + 49)
                    end
                end
            ...
            Tabbox:SetCollapsed(Tabbox.Collapsed, true)
            setmetatable(Tabbox, {
                __index = function(self, key)
                    local active = rawget(self, "ActiveTab")
                    if active then
                        return active[key]
                    end
                    return nil
        ...
        function Tab:AddLeftTabbox(...)
            ...
            if typeof(First) == "table" then
                ...
                local Name, Collapsible, DefaultCollapsed = ...
                return Tab:AddTabbox({ Side = 1, Name = Name, Collapsible = Collapsible, DefaultCollapsed = DefaultCollapsed })
            end
        end
        ...
        function Tab:AddRightTabbox(...)
            ...
            if typeof(First) == "table" then
                ...
                local Name, Collapsible, DefaultCollapsed = ...
                return Tab:AddTabbox({ Side = 2, Name = Name, Collapsible = Collapsible, DefaultCollapsed = DefaultCollapsed })
            end
        end

        function Tab:AddMiddleTabbox(...)
            local First = select(1, ...)
            if typeof(First) == "table" then
                local Info = First
                Info.Side = 3
                ...
                local Name, Collapsible, DefaultCollapsed = ...
                return Tab:AddTabbox({ Side = 3, Name = Name, Collapsible = Collapsible, DefaultCollapsed = DefaultCollapsed })
            end
        end
        ...
        function Tab:Show()
            ...
            if not Tab.IsSearchResultsTab and Library.Searching then
                Library.PreSearchTab = nil
                task.defer(function()
                    if SearchInput then
                        SearchInput.Text = ""
                    end
                end)
                for _, item in ipairs(Library.ReparentedContainers or {}) do
                    item.Container.Parent = item.OriginalParent
                    item.Container.LayoutOrder = item.OriginalLayoutOrder
                    RestoreContainerElements(item.ContainerObject)
                end
                if SearchResultsTab then
                    SearchResultsTab:SetVisible(false)
                end
            end
        ...
        if Side == "Footer" then
            ...
            TabButton.MouseEnter:Connect(function()
                TweenService:Create(TabButton, Library.TweenInfo, {
                    ImageTransparency = Icon and 0.25 or nil,
                    TextTransparency = not Icon and 0.25 or nil,
                }):Play()
            end)
            TabButton.MouseLeave:Connect(function()
                if Library.ActiveTab == Tab then
                    TweenService:Create(TabButton, Library.TweenInfo, {
                        ImageTransparency = Icon and 0 or nil,
                        TextTransparency = not Icon and 0 or nil,
                    }):Play()
                else
                    TweenService:Create(TabButton, Library.TweenInfo, {
                        ImageTransparency = Icon and 0.5 or nil,
                        TextTransparency = not Icon and 0.5 or nil,
                    }):Play()
                end
            end)
    ...
    local function DoesContainerMatch(Container, SearchQuery)
        ...
        if Container.Elements then
            for _, ElementInfo in ipairs(Container.Elements) do
                if ElementInfo.Type == "Dropdown" then
                    if ElementInfo.Text and ElementInfo.Text:lower():find(q, 1, true) then
                        return true
                    end
                    if ElementInfo.Values then
                        for _, Val in ipairs(ElementInfo.Values) do
            ...
            if Container.DependencyBoxes then
                local function checkDepbox(Box)
                    if Box.Elements then
                        for _, ElementInfo in ipairs(Box.Elements) do
                            if ElementInfo.Type == "Dropdown" then
                                if ElementInfo.Text and ElementInfo.Text:lower():find(q, 1, true) then
                                    return true
                                end
                    ...
                    if Box.DependencyBoxes then
                        for _, SubBox in ipairs(Box.DependencyBoxes) do
                            if checkDepbox(SubBox) then
                                return true
                            end
                        end
                    end
                ...
                for _, Box in ipairs(Container.DependencyBoxes) do
                    if checkDepbox(Box) then
                        return true
                    end
                end
            end
        end
        if Container.Tabs then
            for SubTabName, SubTab in pairs(Container.Tabs) do
                if tostring(SubTabName):lower():find(q, 1, true) then
                    return true
                end
                if SubTab.Elements then
                    for _, ElementInfo in ipairs(SubTab.Elements) do
                        if ElementInfo.Type == "Dropdown" then
                            if ElementInfo.Text and ElementInfo.Text:lower():find(q, 1, true) then
                                return true
                            end
    ...
    local function FilterElementsForSearch(Container, query, forceShowAllDropdowns)
        Container.BoxHolder.Visible = true
        if Container.Elements then
            for _, ElementInfo in ipairs(Container.Elements) do
                ElementInfo.Holder.Visible = typeof(ElementInfo.Visible) == "boolean" and ElementInfo.Visible or true
            end
        ...
        if Container.Tabs then
            for _, SubTab in pairs(Container.Tabs) do
                if SubTab.Elements then
                    for _, ElementInfo in ipairs(SubTab.Elements) do
                        ElementInfo.Holder.Visible = typeof(ElementInfo.Visible) == "boolean" and ElementInfo.Visible or true
                    end
    ...
    local function RestoreContainerElements(Container)
        Container.BoxHolder.Visible = true
        if Container.Elements then
            for _, ElementInfo in ipairs(Container.Elements) do
                ElementInfo.Holder.Visible = typeof(ElementInfo.Visible) == "boolean" and ElementInfo.Visible or true
    ...
    local function RunSearch(SearchText)
        local query = SearchText
        local queryActive = query:gsub("%s+", "") ~= ""
        for _, item in ipairs(Library.ReparentedContainers or {}) do
            item.Container.Parent = item.OriginalParent
            item.Container.LayoutOrder = item.OriginalLayoutOrder
            RestoreContainerElements(item.ContainerObject)
        end
        Library.ReparentedContainers = {}
        if not queryActive then
            Library.Searching = false
            if SearchResultsTab then
                SearchResultsTab:SetVisible(false)
            end
            if Library.PreSearchTab then
                local pre = Library.PreSearchTab
                Library.PreSearchTab = nil
                pre:Show()
            else
                local fallbackTab = nil
                for _, Tab in ipairs(Window.TabsList) do
                    if not Tab.IsSearchResultsTab and not Tab.IsKeyTab then
                        fallbackTab = Tab
                        break
                    end
                end
        ...
        if not Library.Searching then
            if Library.ActiveTab and not Library.ActiveTab.IsSearchResultsTab then
                Library.PreSearchTab = Library.ActiveTab
            end
        end
        Library.Searching = true
        ...
        for _, Tab in ipairs(Window.TabsList) do
            if not Tab.IsSearchResultsTab and not Tab.IsKeyTab then
                local tabMatches = Tab.Name:lower():find(query:lower(), 1, true)
                for _, Groupbox in pairs(Tab.Groupboxes or {}) do
                    local match = tabMatches or DoesContainerMatch(Groupbox, query)
                    if match then
                        if not Groupbox.OriginalParent then
                            Groupbox.OriginalParent = Groupbox.BoxHolder.Parent
                            Groupbox.OriginalLayoutOrder = Groupbox.BoxHolder.LayoutOrder
                        end
                        Groupbox.BoxHolder.Parent = SearchResultsTab:GetSideContainer(3)
                        Groupbox.BoxHolder.LayoutOrder = currentLayout
                        currentLayout = currentLayout + 1
                        FilterElementsForSearch(Groupbox, query, tabMatches)
                        table.insert(Library.ReparentedContainers, { Container = Groupbox.BoxHolder, ContainerObject = Groupbox, OriginalParent = Groupbox.OriginalParent, OriginalLayoutOrder = Groupbox.OriginalLayoutOrder })
                    end
                end
                for _, Tabbox in pairs(Tab.Tabboxes or {}) do
                    local match = tabMatches or DoesContainerMatch(Tabbox, query)
                    if match then
                        if not Tabbox.OriginalParent then
                            Tabbox.OriginalParent = Tabbox.BoxHolder.Parent
                            Tabbox.OriginalLayoutOrder = Tabbox.BoxHolder.LayoutOrder
                        end
                        Tabbox.BoxHolder.Parent = SearchResultsTab:GetSideContainer(3)
                        Tabbox.BoxHolder.LayoutOrder = currentLayout
                        currentLayout = currentLayout + 1
                        FilterElementsForSearch(Tabbox, query, tabMatches)
                        table.insert(Library.ReparentedContainers, { Container = Tabbox.BoxHolder, ContainerObject = Tabbox, OriginalParent = Tabbox.OriginalParent, OriginalLayoutOrder = Tabbox.OriginalLayoutOrder })
                    end
                end
            end
        end
        if SearchResultsTab then
            SearchResultsTab:SetVisible(true)
            SearchResultsTab:Show()
        end
    end

    local ShowFAB = Library.IsMobile or WindowInfo.ShowMobileButtons
    if WindowInfo.AutoShow and not Library.ActiveLoading and not ShowFAB then
        task.spawn(Library.Toggle)
    end

    local ShowFABBlock = ShowFAB
    if ShowFABBlock then
        local UpdateMenu
        local BuildMenuButtons
        local justOpenedSearch = false
        local FABIcon
        local UpdateFABIconFlip
        
        local function MakeFABDraggable(UI, DragFrame)
            local Dragging = false
            local StartPos
            local ButtonPos
            local Dragged = false
            ...
            DragFrame.InputBegan:Connect(function(Input)
                ...
                Changed = Input.Changed:Connect(function()
                    ...
                    local ButtonAbsolutePos = UI.AbsolutePosition
                    local ButtonCenterX = ButtonAbsolutePos.X + UI.AbsoluteSize.X / 2
                    
                    local clampedY = math.clamp(UI.AbsolutePosition.Y + UI.AbsoluteSize.Y / 2, 32, ViewportSize.Y - 32)
                    local TargetX, AnchorX
                    local IsLeftHalf = ButtonCenterX < ViewportSize.X / 2
                    if IsLeftHalf then
                        TargetX = UDim2.new(0, 16, 0, clampedY)
                        AnchorX = Vector2.new(0, 0.5)
                    else
                        TargetX = UDim2.new(1, -16, 0, clampedY)
                        AnchorX = Vector2.new(1, 0.5)
                    end
                    
                    if UpdateFABIconFlip then
                        UpdateFABIconFlip(IsLeftHalf)
                    end
                    
                    UI.AnchorPoint = AnchorX
                    TweenService:Create(UI, TweenInfo.new(0.3, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {
                        Position = TargetX
                    }):Play()
                end)
            end)
            ...
            Library:GiveSignal(UserInputService.InputChanged:Connect(function(Input)
                if Dragging and IsHoverInput(Input) then
                    ...
                    if Delta.Magnitude > 5 then
                        if not Dragged and UpdateMenu then
                            UpdateMenu(false)
                        end
                        Dragged = true
                    end
        ...
        FABContainer.Position = IsLeft and UDim2.new(0, 20, 0.5, -24) or UDim2.new(1, -20, 0.5, -24)
        FABContainer.AnchorPoint = IsLeft and Vector2.new(0, 0.5) or Vector2.new(1, 0.5)
        ...
        local SearchOutline, SearchShadow = Library:AddOutline(SearchContainer)
        SearchOutline.Color = Library.Scheme.AccentColor
        Library:AddToRegistry(SearchOutline, {
            Color = "AccentColor",
        })
        table.insert(Library.Corners, New("UICorner", { CornerRadius = UDim.new(0, 8), Parent = SearchContainer }))
        ...
        Library:AddToRegistry(SearchClear, {
            TextColor3 = "FontColor",
        })

        SearchClear.MouseEnter:Connect(function()
            TweenService:Create(SearchClear, Library.TweenInfo, { TextTransparency = 0.25 }):Play()
        end)
        SearchClear.MouseLeave:Connect(function()
            TweenService:Create(SearchClear, Library.TweenInfo, { TextTransparency = 0.5 }):Play()
        end)
        ...
        SearchInput:GetPropertyChangedSignal("Text"):Connect(function()
            ...
            if BuildMenuButtons then
                BuildMenuButtons()
            end
            if UpdateMenu then
                UpdateMenu(MenuExpanded, true)
            end
        end)

        
        local FABButton = New("TextButton", {
            BackgroundColor3 = "BackgroundColor",
            BackgroundTransparency = WindowInfo.PopCat and 0.35 or 0,
            Size = UDim2.fromOffset(48, 48), 
        ...
        table.insert(Library.Corners, New("UICorner", { CornerRadius = UDim.new(0.5, 0), Parent = FABButton }))
        local OutStroke, ShadStroke = Library:AddOutline(FABButton)
        if WindowInfo.PopCat then
            ShadStroke.Enabled = false
            New("UIGradient", {
                Color = ColorSequence.new({
        ...
        UpdateFABIconFlip = function(IsLeftHalf)
            if not FABIcon or not WindowInfo.PopCat then return end
            local dim = WindowInfo.PopCatDimensions
            if IsLeftHalf then
        ...
        local GemIcon = Library:GetCustomIcon(WindowInfo.Icon or "gem")
        local FABColor = WindowInfo.PopCat and Color3.fromRGB(255, 255, 255) or "AccentColor"
        
        FABIcon = New("ImageLabel", {
            BackgroundTransparency = 1,
            Size = WindowInfo.PopCat and UDim2.fromOffset(34, 34) or UDim2.fromOffset(26, 26),
            Position = UDim2.fromScale(0.5, 0.5),
            AnchorPoint = Vector2.new(0.5, 0.5),
            Image = GemIcon and GemIcon.Url or "",
            ImageRectOffset = GemIcon and GemIcon.ImageRectOffset or Vector2.zero,
            ImageRectSize = GemIcon and GemIcon.ImageRectSize or Vector2.zero,
            ImageColor3 = FABColor,
            ZIndex = 103,
            Parent = FABButton,
        })
        ...
        if WindowInfo.PopCat then
            ...
            UpdateFABIconFlip(IsLeft)
        end
        
        local IsDragged = MakeFABDraggable(FABContainer, FABButton)
        ...
        local fadeSeq = 0
        local isFaded = false
        local FADE_IDLE_TIME = 5  
        local FADE_OPACITY = 0.6   
        ...
        local function FadeOut()
            ...
            TweenService:Create(FABButton, TweenInfo.new(0.8, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
                BackgroundTransparency = WindowInfo.PopCat and 0.75 or FADE_OPACITY
            }):Play()
            TweenService:Create(FABIcon, TweenInfo.new(0.8, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
                ImageTransparency = FADE_OPACITY
            }):Play()
            for _, Stroke in FABButton:GetChildren() do
                if Stroke:IsA("UIStroke") then
                    TweenService:Create(Stroke, TweenInfo.new(0.8, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
                        Transparency = WindowInfo.PopCat and 1 or FADE_OPACITY
                    }):Play()
                end
            end
        end
        ...
        local function FadeIn()
            ...
            TweenService:Create(FABIcon, TweenInfo.new(0.3, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
                ImageTransparency = 0
            }):Play()
            for _, Stroke in FABButton:GetChildren() do
                if Stroke:IsA("UIStroke") then
        ...
        local function ResetFadeTimer()
            ...
            fadeSeq = fadeSeq + 1
            local mySeq = fadeSeq
            task.delay(FADE_IDLE_TIME, function()
                if fadeSeq == mySeq then
                    FadeOut()
                end
            end)
        end
        ...
        FABButton.MouseEnter:Connect(function()
            ...
            TweenService:Create(FABIcon, TweenInfo.new(0.25, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {
                Size = WindowInfo.PopCat and UDim2.fromOffset(40, 40) or UDim2.fromOffset(28, 28)
            }):Play()
        end)
        ...
        UpdateMenu = function(Expand, Force)
            if MenuExpanded == Expand and not Force then return end
            MenuExpanded = Expand
            ...
            local R = 75 + math.max(0, (TotalButtons - 5) * 8)
            
            for idx, item in ipairs(MenuButtons) do
                local Btn = item.Button
                local Stroke = item.Stroke
                local IconLabel = item.IconLabel
                ...
                if Expand then
                    ...
                    if TotalButtons > 1 then
                        spacing = math.min(35, 140 / (TotalButtons - 1))
                    end
                    local angle
                    if IsLeftHalf then
                        angle = math.rad((idx - (TotalButtons + 1) / 2) * spacing)
                    else
                        angle = math.rad(180 - (idx - (TotalButtons + 1) / 2) * spacing)
                    end
                    
                    local targetX = R * math.cos(angle)
                    local targetY = R * math.sin(angle)
                    
                    local targetStrokeColor = Library.Scheme.OutlineColor
                    local targetIconColor = Library.Scheme.FontColor
                    local targetTransparency = 0.15
                    
                    if action.Name == "Lock Drag" and Library.CantDragForced then
                        targetStrokeColor = Library.Scheme.AccentColor
                        targetIconColor = Library.Scheme.AccentColor
                        targetTransparency = 0
                    elseif action.Name == "Search UI" and SearchContainer and SearchContainer.Visible then
                        targetStrokeColor = Library.Scheme.AccentColor
                        targetIconColor = Library.Scheme.AccentColor
                        targetTransparency = 0
                    end
        ...
        BuildMenuButtons = function()
            ...
            table.clear(MenuButtons)
            
            local Actions = {}
            
            local function DoesTabMatch(Tab, Query)
                local q = Query:lower()
                if Tab.Name:lower():find(q, 1, true) then
                    return true
                end
                    ...
                    for _, Groupbox in pairs(Tab.Groupboxes) do
                        if Groupbox.Elements then
                            for _, ElementInfo in pairs(Groupbox.Elements) do
                                ...
                                if ElementInfo.SubButton and ElementInfo.SubButton.Text and ElementInfo.SubButton.Text:lower():find(q, 1, true) then
                                    return true
                                end
                            end
                        end
                        if Groupbox.DependencyBoxes then
                            local function checkDepbox(Box)
                                if Box.Elements then
                                ...
                                if Box.DependencyBoxes then
                                    for _, SubBox in pairs(Box.DependencyBoxes) do
                                        if checkDepbox(SubBox) then
                                            return true
                                        end
                                    end
                            ...
                            for _, Box in pairs(Groupbox.DependencyBoxes) do
                                if checkDepbox(Box) then
                                    return true
                                end
                            end
                ...
                if Tab.Tabboxes then
                    for _, Tabbox in pairs(Tab.Tabboxes) do
                        if Tabbox.Tabs then
                            for SubTabName, SubTab in pairs(Tabbox.Tabs) do
                                if tostring(SubTabName):lower():find(q, 1, true) then
                                    return true
                                end
                                if SubTab.Elements then
            ...
            local QueryActive = SearchQuery:gsub("%s+", "") ~= ""
            local AddedTabs = 0
            
            for _, Tab in ipairs(Window.TabsList) do
                if (Tab.TabVisible or Tab.Name == "Settings") and Tab.ShowOnFloating ~= false then
                    local Match = true
                    if QueryActive then
                        Match = DoesTabMatch(Tab, SearchQuery)
                    end
                    if Match then
            ...
            table.insert(Actions, {
                Name = "Lock Drag",
                Icon = Library.CantDragForced and "unlock" or "lock",
                Func = function(self)
                    Library.CantDragForced = not Library.CantDragForced
                    local LockIcon = Library:GetCustomIcon(Library.CantDragForced and "unlock" or "lock")
                    if LockIcon then
                        self.Button.IconLabel.Image = LockIcon.Url
                        self.Button.IconLabel.ImageRectOffset = LockIcon.ImageRectOffset
                        self.Button.IconLabel.ImageRectSize = LockIcon.ImageRectSize
                    end
                    if Library.CantDragForced then
                        TweenService:Create(self.Button.UIStroke, TweenInfo.new(0.3, Enum.EasingStyle.Quad), {
                            Color = Library.Scheme.AccentColor,
                            Thickness = 2,
                        }):Play()
                        TweenService:Create(self.Button.IconLabel, TweenInfo.new(0.3, Enum.EasingStyle.Quad), {
            ...
            for _, action in ipairs(Actions) do
                ...
                table.insert(Library.Corners, New("UICorner", { CornerRadius = UDim.new(0.5, 0), Parent = Btn }))
                local Stroke = New("UIStroke", { Color = "OutlineColor", Transparency = 1, Parent = Btn })
                
                local IconData
                if typeof(action.Icon) == "table" then
                    IconData = action.Icon
                else
                    IconData = Library:GetCustomIcon(action.Icon)
                end
                ...
                Btn.MouseEnter:Connect(function()
                    ...
                    if MenuExpanded then
                        ...
                        TweenService:Create(IconLabel, TweenInfo.new(0.25, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {
                            Size = UDim2.fromOffset(20, 20),
                            ImageColor3 = Library.Scheme.AccentColor,
                            ImageTransparency = 0
                        }):Play()
                        TweenService:Create(Stroke, TweenInfo.new(0.25, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {
                            Color = Library.Scheme.AccentColor
                        }):Play()
                    end
                end)
                ...
                Btn.MouseLeave:Connect(function()
                    ...
                    if MenuExpanded then
                        ...
                        local targetIconColor = Library.Scheme.FontColor
                        local targetStrokeColor = Library.Scheme.OutlineColor
                        local targetIconTransparency = 0.3
                        if action.Name == "Lock Drag" and Library.CantDragForced then
                            targetIconColor = Library.Scheme.AccentColor
                            targetStrokeColor = Library.Scheme.AccentColor
                            targetIconTransparency = 0
                        elseif action.Name == "Search UI" and SearchContainer and SearchContainer.Visible then
                            targetIconColor = Library.Scheme.AccentColor
                            targetStrokeColor = Library.Scheme.AccentColor
                            targetIconTransparency = 0
                        end
                        TweenService:Create(IconLabel, TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
                            Size = UDim2.fromOffset(18, 18),
                            ImageColor3 = targetIconColor,
        ...
        task.defer(BuildMenuButtons)
        
        local clickCount = 0
        local clickSequenceNum = 0
        local fabClicked = false
        FABButton.MouseButton1Click:Connect(function()
            ...
            if WindowInfo.PopCat then
                ...
                TweenService:Create(FABIcon, TweenInfo.new(0.08, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
                    Size = UDim2.fromOffset(currentIconSize.X.Offset * 0.75, currentIconSize.Y.Offset * 1.15)
                }):Play()
            ...
            clickSequenceNum = currentSeq
            
            task.delay(0.25, function()
                if clickSequenceNum == currentSeq then
                    if clickCount == 1 then
                        UpdateMenu(not MenuExpanded)
        ...
        Library:GiveSignal(UserInputService.InputBegan:Connect(function(Input)
            if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
                if MenuExpanded then
                    task.defer(function()
                        if not MenuExpanded then return end
                        if isHoveringMenu then return end
                        if fabClicked then return end
                        
                        local clickedInside = false
                        for _, item in ipairs(MenuButtons) do
                            if item.Button.Visible and Library:MouseIsOverFrame(item.Button, Input.Position) then
                                clickedInside = true
                                break
                            end
                        end
                ...
                if SearchContainer.Visible then
                    task.defer(function()
                        if not SearchContainer.Visible then return end
                        if justOpenedSearch then return end
                        
                        local clickedInside = Library:MouseIsOverFrame(SearchContainer, Input.Position)
                        if not clickedInside and SearchInput.Text:gsub("%s+", "") == "" then
