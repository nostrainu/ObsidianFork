...
function Library:CreateWindow(WindowInfo)
    ...
    local function FilterElementsForSearch(Container, query, forceShowAllDropdowns)
        ...
        if Container.Elements then
            for _, ElementInfo in ipairs(Container.Elements) do
                if ElementInfo.Type == "Dropdown" then
                    local match = forceShowAllDropdowns
                    if not match then
                        if ElementInfo.Text and ElementInfo.Text:lower():find(q, 1, true) then
                            match = true
                        end
                    ...
                    ElementInfo.Holder.Visible = match and (typeof(ElementInfo.Visible) == "boolean" and ElementInfo.Visible or true)
                else
                    ElementInfo.Holder.Visible = false
                end
            end
    ...
    if ShowFABBlock then
        ...
        local XIcon = Library:GetCustomIcon("x")
        if XIcon then
            SearchClear.Image = XIcon.Url
            SearchClear.ImageRectOffset = XIcon.ImageRectOffset
            SearchClear.ImageRectSize = XIcon.ImageRectSize
        end
        ...
        SearchClear.MouseEnter:Connect(function()
            TweenService:Create(SearchClear, Library.TweenInfo, { ImageTransparency = 0.25 }):Play()
        end)
        SearchClear.MouseLeave:Connect(function()
            TweenService:Create(SearchClear, Library.TweenInfo, { ImageTransparency = 0.5 }):Play()
        end)
